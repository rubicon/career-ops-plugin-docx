...
/**
 * Open an untitled section at the head of the document. A CV may put a ### or
 * #### above its first ## heading; that nesting has no section to attach to, and
 * dropping it would lose the content with nothing in the output to show for it.
 * The section renders without a header rule, so the nesting simply leads.
 *
 * The `implicit` flag is what the renderer keys on. Testing the title instead
 * would also swallow the header of an explicit `##` whose text strips to empty,
 * which the author did ask for and which still carries the divider rule.
 * @param {{sections: Array}} cv
 * @returns {{title: string, implicit: boolean, blocks: Array}}
 */
function openImplicitSection(cv) {
  const section = { title: '', implicit: true, blocks: [] };
...
export function parseCvMarkdown(markdown) {
  ...
  for (const raw of lines) {
    ...
    if (h4) {
      ...
      if (!section) {
        section = openImplicitSection(cv);
        seenSection = true;
      }
      if (!entry) {
        entry = {
...
function renderBody(cv) {
  ...
  for (const section of cv.sections) {
    // Only the implicit leading section goes without a header: it was never
    // written, so there is nothing to head. An explicit ## keeps its header
    // (and the rule under it) even when the heading text strips to empty.
    if (!section.implicit) out += styledParagraph('SectionHeader', section.title.toUpperCase());
